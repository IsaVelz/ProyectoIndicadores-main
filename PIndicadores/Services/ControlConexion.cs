    ...
    public class ControlConexion
    {
        ...
        public DbParameter CrearParametro(string nombre, object? valor)
        {
            try
            {
                ...
                {
                    "SqlServer" => new SqlParameter(nombre, valor ?? DBNull.Value), // Crea un parámetro para SQL Server / evita inyecciones de datos
        ...
        public object? EjecutarEscalar(string consultaSQL, DbParameter[] parametros)
        {
            using (var comando = _conexionBd.CreateCommand())
            {
